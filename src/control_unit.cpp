...
namespace softcpu {
namespace {
constexpr std::uint8_t kStackRegisterIndex =
    static_cast<std::uint8_t>(kRegisterCount - 1); // R7 is SP
...
// Helper to read a register value
...
// Helper to write a register value
void writeRegister(RegisterFile &regs, std::uint8_t index,
                   std::uint16_t value) {
  if (index >= kRegisterCount) {
    return;
  }
...
// Map port ID to memory-mapped I/O address
...
void ControlUnit::reset() { registers_.reset(); }
...
Operand ControlUnit::resolveOperand(const OperandDescriptor &descriptor,
                                    std::uint16_t &pc) {
  Operand operand;
...
// Helper to read the value of an operand
std::uint16_t readOperandValue(Bus &bus, const RegisterFile &regs,
...
// Helper to write a value to an operand destination
void writeOperandValue(Bus &bus, RegisterFile &regs, const Operand &operand,
                       std::uint16_t value) {
  switch (operand.type) {
  case OperandType::Register:
...
bool ControlUnit::execute(const DecodedInstruction &inst, bool) {
  ...
  case Opcode::SHL: {
    ...
    const auto shift = static_cast<std::uint8_t>(
        readOperandValue(bus_, registers_, inst.operand_b) & 0xFF);
  ...
  case Opcode::ADJSP: {
    const auto delta = static_cast<std::int16_t>(
        readOperandValue(bus_, registers_, inst.operand_a));
