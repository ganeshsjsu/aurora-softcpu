...
std::uint16_t Bus::read16(std::uint16_t address) const {
    if (auto* dev = findDevice(address)) {
        ...
        const std::uint8_t high = dev->read(dev->offset(static_cast<std::uint16_t>(address + 1)));
...
void Bus::write16(std::uint16_t address, std::uint16_t value) {
    if (auto* dev = findDevice(address)) {
        ...
        dev->write(dev->offset(static_cast<std::uint16_t>(address + 1)), static_cast<std::uint8_t>((value >> 8) & 0xFF));
        return;
    }
