...
void Emulator::loadImage(const std::vector<std::uint8_t>& image, std::uint16_t origin) {
...
RegisterFile& Emulator::registers() {
    return cpu_->registers();
}

const RegisterFile& Emulator::registers() const {
    return cpu_->registers();
}
