...
namespace {
constexpr std::uint8_t hi(std::uint16_t value) { return static_cast<std::uint8_t>((value >> 8) & 0xFF); }
constexpr std::uint8_t lo(std::uint16_t value) { return static_cast<std::uint8_t>(value & 0xFF); }
}  // namespace
...
void Memory::loadBlock(const std::vector<std::uint8_t>& data, std::uint16_t origin) {
